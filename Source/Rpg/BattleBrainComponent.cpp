...
void UBattleBrainComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (RunActiveTurn())
	{
		UE_LOG(LogTemp, Warning, TEXT("turn over %d"),TurnOrder.Num());
		//start next turn
...
void UBattleBrainComponent::ReCalcChrsTurns(ABattlePawnBase* inPawn)
{
	...
	while (TurnNos.Num() < 3)
	{
		...
		AddNewTurn(CurrentTurns[0]->MyBattlePawn, CurrentTurns[0]->ChrName, HighestTurnNo+1, CurrentTurns[0]->MyBattlePawn->mainCharInfo.Speed);
...
bool UBattleBrainComponent::RunPlayersTurn()
{
	...
	testOperationCompleted = true;
		...
		return testOp2 = AttackMagic(attaaaaTarget);
